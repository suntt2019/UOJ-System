...
<?php if (isset($contest['extra_config']['contest_type']) && $contest['extra_config']['contest_type']=='ACM'):?>
	<ul>
		...
		<li>比赛中途提交后，可以看到<strong>第一个不是Accepted测试点</strong>的结果。（不显示详细信息，若所有测试点均Accepted则显示Accepted）</li>
		<li>比赛结束前一段时间会进行封榜，封榜期间的提交会一直显示为评测中，只有提交者可见结果。</li>
		<li>比赛结束后会进行最终测试，最终测试后的排名为最终排名。</li>
		<li>比赛排名按分数（通过题目数）为第一关键字，完成题目的总时间为第二关键字。完成题目的总时间等于<strong>完成每道题所花时间与罚时之和</strong>（无视掉爆零的题目）。</li>
